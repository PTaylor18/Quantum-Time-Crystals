...
function Mz_evolve(N::Int, nsteps::Int, γX::Float64, γZZ::Float64)
    ...
    if (γX > 1E-8) && (γZZ > 1E-8)
        ...
        for n in 1:nsteps
            ...
            measured_Mz = Mz_mixed(N, ρ)
            println(measured_Mz)
            append!(Mz_list, measured_Mz)
        end
    else
...
nsteps = 5 # number of Trotter steps
γX = 0.00001 # single-qubit error rate
γZZ = 0.00001 # two-qubit error rate
