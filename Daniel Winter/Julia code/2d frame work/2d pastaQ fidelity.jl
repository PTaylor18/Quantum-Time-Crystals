...
function Mz_evolve(N::Int, nsteps::Int, γX::Float64, γZZ::Float64)
    ...
    if (γX > 1E-8) && (γZZ > 1E-8)
        ...
        for n in 1:nsteps
            if one_d == true
                ρ = runcircuit(ρ, flip(N), cutoff = res; noise = ("amplitude_damping", (γ = γX,))) # , apply_dag=true
                #results = measure(ρ, "Z"); println(results)
                #normalize!(ρ) # normalize the density operator
                ρ = runcircuit(ρ, ZZ_layer(N), cutoff = res; noise = ("amplitude_damping", (γ = γZZ,))) # , apply_dag=true
                normalize!(ρ) # normalize the density operator
                ...
                ρ = runcircuit(ρ, ZZ_layer(N), cutoff = res; noise = ("amplitude_damping", (γ = γZZ,))) # , apply_dag=true
                #normalize!(ρ) # normalize the density operator
                ρ = runcircuit(ρ, ZZ_3layer(N), cutoff = res; noise = ("amplitude_damping", (γ = γZZ,))) # , apply_dag=true
                #normalize!(ρ) # normalize the density operator
                ρ = runcircuit(ρ, ZZ_4layer(N), cutoff = res; noise = ("amplitude_damping", (γ = γZZ,))) # , apply_dag=true
                normalize!(ρ) # normalize the density operator
                #println(PastaQ.array(ρ))
                #measurement = Mz_mixed(N, ρ)
                isodd(n) ? (measurement = -1. * fidelity(ρ, ρ1)) : (measurement = 1. * fidelity(ρ, ρ0))
...
for runs = 1
    for q = qmin:2:qmax # Number of qubits for each
        for step = 50
            one_d = true
