...
Ystring = chain(N, prod([put(N, i=>Y) for i=1:N]))
...
plot_name = @Name Continuous_TC_modwave
...
for j = N # Number of qubits for each
    for step = 10000
        ...
        for i = 0.00001
            ...
            fig=Plots.plot(t_vec1[1:10000], Mz_vec1[1:10000], linetype=:steppre, xlabel = "Time/ period of driving field", ylabel = " \n"*"Magnetisation / fraction of maximum value\n and orientation", legend = false)
            Plots.title!(title1)
            #Plots.savefig(figpath1*title1*".png") #Saves the plots onto my computer but requires me to make a folder

            label=title1;

            #save_plot(plot_name, j, step, fig, label) # Saves the plots to github
            display(fig)
