...
function op(::OpName"openHeisH", ::SiteType"S=1/2", s1::Index, s2::Index; τ, S)
  ampo =  + 1.0 * op("Sz", s1)*op("Sz",s2) + (S/2) * (op("S+", s1)*op("S-", s2)
          + (S/2) * op("S-", s1)*op("S+", s2))
...
function Ckop(s, C, N, nk)
  ...
  for j in 1:N
    sj = s[j]
    coeff = 101 * exp(im * (2*pi)^2 * nk * j/N) #* N^0.5
    ampoj = coeff * op(C, sj)
...
function op(::OpName"Ck", ::SiteType"S=1/2", s1::Index; C, N, nk)
...
function CdagexpCexp(ψ, s, t, N, totalt, S, nk)
  ...
  B = ops([("openHeisH", (n, n + 1), (τ=-t * im, S, )) for n in 1:(N - 1)], s)
  Bdag = ops([("openHeisH", (n, n + 1), (τ=t * im, S, )) for n in 1:(N - 1)], s)
  ...
  ρ1 = apply(Ckop(s, "S+", N, -nk), ρ1; cutoff=1E-12)
...
function expCexpCdag(ψ, s, t, N, totalt, S, nk)
  ...
  #Bdag = op(sid, "S+")
  #B = excite(N, site)
  Adag = ops([("openHeisH", (n, n + 1), (τ=t * im, S,)) for n in 1:(N - 1)], s)
  A = ops([("openHeisH", (n, n + 1), (τ=-t * im, S,)) for n in 1:(N - 1)], s)
  #B = op(sid, "S-")
...
Odag = Ckop(sites, "S+", N, nk)
#Odag1 = ops([("Ck", (n), ("S+", N, nk, ))  for n in 1:(N)], sites)
res = apply(Odag, psi)
res1 = apply(O, res)
