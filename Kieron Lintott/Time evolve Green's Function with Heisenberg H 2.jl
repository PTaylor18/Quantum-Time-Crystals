...
function Ckop(ρ1, C, N, nk, dag)
  ...
  dag determines whether it's C_k dagger or not (the appropriate
  operator in space is passed as an appropriate argument).
  ...
  if dag == true
    for j in 1:N
      Ck += exp(-im * (2*pi)^2 * nk * j/N) * Cop(ρ1, j, C)
    end
  else
    for j in 1:N
      Ck += exp(-im * (2*pi)^2 * nk * j/N) * Cop(ρ1, j, C)
